...
# 应用配置
appid = "YOUR_VOLC_TTS_APPID"  # 应用ID
token = "YOUR_VOLC_TTS_TOKEN"  # 访问令牌
cluster = "volcano_tts"  # 集群名称
voice_type = "YOUR_VOLC_TTS_VOICE_TYPE"  # 语音类型
...
async def send_tts_request(text):
    ...
    """
    # 使用默认路径
    return await send_tts_request_to_path(text, "output.mp3")

async def send_tts_request_to_path(text, file_path):
    """
    发送TTS请求到服务器并保存音频文件到指定路径
    :param text: 待合成的文本
    :param file_path: 保存音频文件的路径
    :return: 保存的音频文件路径
    """
    # 复制请求模板并填充数据
...
def handle_tts_request(req):
    ...
    try:
        # 使用绝对路径保存音频文件
        audio_path = "/home/abot/8C8PE4/src/TTS_audio/scripts/output.mp3"

        # 运行异步函数
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(send_tts_request_to_path(TEXT, audio_path))
        loop.close()

        # 播放生成的音频
        rospy.loginfo(f"音频保存至: {audio_path}")
        os.system('mplayer "%s"' % audio_path)
