...
def face_rec():
    read_dir = "/home/abot/8C8PE4/src/face_pkg/scripts/data"
    [names, X, y] = read_images(read_dir)
    y = np.asarray(y, dtype=np.int32)
    model = cv2.face_EigenFaceRecognizer.create()
    model.train(np.asarray(X), np.asarray(y))
    
    face_cascade = cv2.CascadeClassifier('/home/abot/8C8PE4/src/face_pkg/scripts/cascades/haarcascade_frontalface_default.xml')
